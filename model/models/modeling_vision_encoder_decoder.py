...
class VisionEncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        self.mode = mode
        if self.mode=='emo':
            self.cost_embedding = self.decoder.output_projection.weight.data.clone()
            # self.cost_embedding = self.decoder.lm_head.weight.data.clone()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            mle_loss = loss_fct(logits.reshape(-1, self.decoder.config.vocab_size), labels.reshape(-1))

        if self.mode == 'mle' and labels is not None:
            loss = mle_loss

        if self.mode == 'emo' and labels is not None:
            ...
            labels_tmp = labels.clone()
            labels_tmp[labels_tmp==(-100)] = 0
            one_hot = torch.nn.functional.one_hot(labels_tmp, num_classes=self.decoder.config.vocab_size)
            stable_onehot = (one_hot+1e-15) / torch.linalg.vector_norm((one_hot+1e-15), ord=1, dim=-1, keepdim=True) # (bsz*seq_len, vocab_size)
            embedding_matrix = self.cost_embedding # (vocab_size, hidden_size)
            embedding_matrix = embedding_matrix / torch.linalg.vector_norm(embedding_matrix, ord=2, dim=1, keepdim=True)

            # embedding_matrix = embedding_matrix.to(stable_onehot.device)
            ...
            cosine_dist = cosine_dist.reshape(bsz, seq_len)
            emo_loss = cosine_dist.reshape(-1)
            loss = (mle_loss + (mle_loss / (emo_loss+1e-10)).detach() * emo_loss) * 0.5
