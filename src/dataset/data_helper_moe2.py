...
class trocrDataset(Dataset):
    ...
    def __init__(self, paths, processor, max_target_length=128, transformer=lambda x:x,
        ...
        # self.transformer = transforms.Grayscale(num_output_channels=1)
    ...
    def __getitem__(self, idx):
        ...
        if self.types[idx] == 0:#num
            priori = torch.tensor(self.num_priori)
        else:#text
            priori = torch.tensor(self.text_priori)

        encoding = {"pixel_values": pixel_values.squeeze(), "labels": torch.tensor(labels), "priori" : priori}
