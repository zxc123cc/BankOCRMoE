...
def decode_text_with_probs(tokens, score, vocab, vocab_inp):
    ##decode trocr
    s_start = vocab.get('<s>')
    s_end = vocab.get('</s>')
    unk = vocab.get('<unk>')
    ...
    text = ''
    probs = []
    for tk, sc in zip(tokens, score):
        if tk not in [s_end, s_start, pad, unk]:
            text += vocab_inp[tk]
            a = round(np.exp(sc.cpu().numpy()), 2)
            probs.append(str(a))

    return text, ",".join(probs)
